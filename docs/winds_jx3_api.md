...
推送只覆盖当前服务实例在线连接，不提供离线补发、逐客户端确认或跨实例转发。微博首次监听只建立基线，服务重启不补发历史推文，同一推文 ID 不重复推送；剑三新闻、公告和区服常规采集首次只建立基线；更新包监听优先恢复已持久化版本，发现更高版本后按实际升级链推送，仅无已知版本且无第三方通知时静默建立首次基线。第三方连接收到的有效文章通知按 5.7、开服通知按 5.9、更新包通知按 5.10 核验和去重，不受首次基线限制。

剑三新闻和公告以 30 秒为基准，成功后等待加入 ±20% 随机抖动（默认约 24–36 秒），避免固定探测相位；这不保证缩短平均发现时间。区服探测间隔 10 秒，补丁间隔 30 秒；实际周期还包含本轮执行耗时。采集任务统一交给应用异步执行器，调度器只触发任务；同一来源不会重叠执行。常规区服轮询按网关探测完成顺序处理，需连续两次明确结果确认变化；第三方开服通知会独立触发一次即时异步核验，规则见 5.9。
...
新闻和公告正文复用应用异步执行器，每轮最多六个正文请求并行，列表与第三方通知共享八个正文请求的总并发上限，按完成顺序入库和推送。单篇正文失败不丢弃其他成功文章；本轮仍报告失败并保留上次完整扫描基线。连续失败的等待保持原有退避，默认为 30、60、120、240、480、600 秒，成功后恢复抖动节奏。

JX3API `action=2002`、`status=success` 的 `detail.type="官方新闻"` / `"官方公告"` 通过现有事件 WSS 协同监听。客户端校验非空标题、`MM/dd` 日期及官网链接身份后，立即异步请求对应官网详情，不等待定时轮询、列表更新或轮询退避；仍发布现有 `jx3.news.updated` / `jx3.maintenance.updated`，不增加下游事件类型。支持官网旧式 `show-<catid>-<id>-1.html`、`announce/gg.html?id=<id>` 以及当前文章详情路由。

第三方日期不含年份和时分秒，不能代替 `publishedAt`，正文和准确发布时间必须从官网取得；核验或入库失败不广播，待重试身份保留到后续轮询重试（进程内）；已入库但事件发布器抛出异常时，重试原事件编号，避免同内容去重吞掉待发布事件。同篇在途通知合并，成功结果复用原文章身份和业务比较规则，重复通知、后续轮询及重启后的相同内容不会重复推送。请求序号防止迟到旧结果覆盖已保存的新结果。历史文章内容真正变化仍按 `updated` 发布。

排查延迟时，核对 `收到第三方文章通知`、`剑三文章已保存` 及相同事件编号的 WS 广播日志。保存日志提供来源、官方发布时间、发现时间和时间差；`publishedAt` 到 `recordedAt` 的差值不等于纯网络或推送耗时，可能包含官网标注时间与实际可见时间的差异。将 `com.hidewnd.winds.jx3.scheduler.Jx3PollingScheduler` 设为 DEBUG 可查看各轮执行耗时、下一次等待和连续失败数。
...
更新包协同监听复用 `winds.jx3.socket` 的常驻连接，处理 JX3API `action=2003`、`status=success`，上游格式如下：
...
```json
{
  ...
  "status": "success",
  "detail": {
    "now_version": "1.5.0.9957",
    "new_version": "1.5.0.9958",
    "package_num": 1,
    "package_size": "111.27 MB"
  }
}
```

通知必须包含递增的四段数字版本、正整数包数及非空大小字符串。收到后立即提交异步核验，不等待下一次定时触发；补丁扫描与保存共用串行版本处理流程，同一目标的在途通知合并。官方升级链、文件 `Content-Length` 和 `Last-Modified` 仍是包数、精确总字节数和更新时间的依据，不将第三方展示大小反算成精确字节。单次扫描最多四个 HEAD 请求并行，不下载补丁包体。

官网尚未同步通知目标时保留通知，后续轮询重试；较高的未核实通知不会挡住官网已可用的更新。重启恢复完整持久化版本，继续处理到新版本的实际升级链；同版本重复通知不再推送。若同版本此前只保存了静默基线，收到通知后会按通知起点核验并发布一次。入库失败不广播；进程内发布器异常后重试原事件编号，不创建第二个未落库事件。上游无时间戳，`occurredAt` 使用首次发现/接收时间，不冒充官方更新时间。

`jx3_records` 中 `key=patch` 的 `state.packages` 只保留相关补丁文件及 from/to：首次无起点时仅保留直接到当前版本的包，`packageScope=latestVersion`；后续更新或通知核验仅保留本次实际升级链，`packageScope=upgradeChain`。`packageCount`、`totalBytes` 和 `updatedAt` 均只统计该数组。旧 `packageScope=manifest` 记录兼容读取，历史修订不自动批量改写或删除。

排查时区分：`updatedAt` 是相关官方文件的最新修改时间，`recordedAt` 是数据库入库时间，`event=null` 表示该记录没有生成推送事件。日志包含第三方通知接收、持久化版本恢复、首次静默基线，以及状态保存和事件编号；将 `com.hidewnd.winds.jx3.service.impl.PatchMonitorServiceImpl` 设为 DEBUG 可查看轮次耗时及尚未同步的目标版本。实际 WS 发送仍按同一事件编号核对广播日志。
